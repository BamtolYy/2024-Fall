...
for i = 1:3
    ...
    if(strcmp(codeType,'rand'))
        ...
        disp(max(R12/Ns))
        if Nr==1
            %Extract auto-data outside of main peak
            peakWidth=round(Tc/delt); % Width of the first peak in samples
            [~,peakIndex] = max(R1); % Find the index at Peak
            outerPoints=R1(peakIndex+peakWidth:end);
            Ratio_auto_rand = max(outerPoints)/max(R1);
        end
    elseif(strcmp(codeType,'pi'))
        ...
        if Nr==1
            ...
            [~,peakIndex] = max(R1); % Find the index at Peak
            outerPoints=R1(peakIndex+peakWidth:end);
            Ratio_auto_pi = max(outerPoints)/max(R1);
        end
    elseif(strcmp(codeType,'mseq'))
        ...
        if Nr==1
            ...
            [~,peakIndex] = max(R1); % Find the index at Peak
            outerPoints=R1(peakIndex+peakWidth:end);
            Ratio_auto_mseq = max(outerPoints)/max(R1);
        end
    end
...
%Find ratios to determine best auto and cross
